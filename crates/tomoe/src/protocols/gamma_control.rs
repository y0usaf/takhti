//! wlr-gamma-control-unstable-v1: per-output gamma ramps, the protocol
//! night-light daemons (wlsunset, gammastep) speak.
//!
//! Niri's implementation (`ref/niri/src/protocols/gamma_control.rs`),
//! verbatim in shape: one active control per output (a second
//! `get_gamma_control` gets `failed()` immediately), the ramp arrives as
//! 3 × gamma_size u16s over an fd, and every error path both fails the
//! resource and resets the output's gamma so a crashed daemon never
//! leaves the screen tinted. The actual LUT programming lives behind
//! [`GammaControlHandler`] in the tty backend; winit reports no gamma
//! support and clients get `failed()`.

// `Output` keys hash by their inner Arc pointer; the atomics inside never
// affect the hash. Same false positive niri hits with this map.
...
pub trait GammaControlHandler {
    ...
    /// Ramp size per channel for the output, or None when gamma is
    /// unsupported (winit, LUT-less CRTC).
    fn get_gamma_size(&mut self, output: &Output) -> Option<u32>;
    /// Program (Some) or reset (None) the output's gamma. None result
    /// fails the client's control object.
...
impl GammaControlManagerState {
    ...
    /// The output is gone; tell its controller (the daemon re-acquires on
    /// the next hotplug) and forget it.
    ...
    D: 'static,
{
    ...
    fn destroyed(
        ...
        // Control object gone (daemon exit): reset the output's gamma.
        let gamma_controls = &mut state.gamma_control_manager_state().gamma_controls;
        let Some((output, _)) = gamma_controls.iter().find(|(_, x)| *x == resource) else {
            return;
        };
