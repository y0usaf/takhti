//! Render-element enum macro (niri's `niri_render_elements!`).
//!
//! smithay's `render_elements!` implements `RenderElement<R>` generically,
//! which shader elements can't satisfy: their `draw` needs the concrete
//! `GlesFrame` (`override_default_tex_program`). This macro generates the
//! `Element` impl generically but `RenderElement` only for tomoe's two real
//! renderers — `GlesRenderer` (winit, capture) and `TtyRenderer` (DRM
//! multi-GPU) — which is where rendering actually happens.

#[macro_export]
macro_rules! tomoe_render_elements {
    // The two callable variants: with <R> and without <R>. They include From impls because
    // nested repetitions ($type and $variant with + and $R with ?) don't work properly.
    ($name:ident<R> => { $($variant:ident = $type:ty),+ $(,)? }) => {
        $crate::tomoe_render_elements!(@impl $name () ($name<R>) => { $($variant = $type),+ });

        $(impl<R: $crate::render::TomoeRenderer> From<$type> for $name<R> {
            fn from(x: $type) -> Self {
                Self::$variant(x)
            }
        })+
    ...
    ($name:ident => { $($variant:ident = $type:ty),+ $(,)? }) => {
        $crate::tomoe_render_elements!(@impl $name ($name) () => { $($variant = $type),+ });
    ...
    // The internal variant that generates most of the code. $name_no_R and $name_R are
    // necessary for the impl RenderElement<SomeRenderer> for $name<SomeRenderer>: since $R
    // does not appear in this line, we cannot condition based on $R like elsewhere, so we
    // condition on duplicate names instead: $($name_R<SomeRenderer>)? $($name_no_R)?.
    (@impl $name:ident ($($name_no_R:ident)?) ($($name_R:ident<$R:ident>)?) => { $($variant:ident = $type:ty),+ }) => {
        #[allow(clippy::large_enum_variant)]
        #[derive(Debug)]
        pub enum $name$(<$R: $crate::render::TomoeRenderer>)? {
            $($variant($type)),+
        }

        impl$(<$R: $crate::render::TomoeRenderer>)? smithay::backend::renderer::element::Element for $name$(<$R>)? {
